...
class DetThread(QThread):
    ...
    def run(self,
            ...
            classes=None,  # 只显示某种分类: --class 0, or --class 0 2 3
        ...
        try:
            ...
            while True:
                ...
                if self.is_continue:
                    ...
                    if self.rate_check and flag:
                        jump_count += 1
                        if jump_count % self.rate != 0:
                            continue
                    # 刷新帧率
                    ...
                    pred = non_max_suppression(pred, self.conf_thres, self.iou_thres,
                                               classes, self.agnostic_nms, max_det=max_det)
                    # Process detections
                    ...
                    # print(type(im0s))
                    if statistic_dic[names[1]] > 0 or statistic_dic[names[2]] > 0:
                        self.send_notice.emit("请佩戴好口罩")
                    else:
                        self.send_notice.emit("")
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        # 不想看到空白可以用FramelessWindowHint模式，但是需要重写鼠标事件才能通过鼠标拉伸窗口
        ...
        self.det_thread.send_notice.connect(lambda x: self.notice.setText(x))
        ...
        self.checkBox_nms.clicked.connect(self.check_nms)
        self.load_setting()
    ...
    def check_nms(self):
        if self.checkBox_nms.isChecked():
            self.det_thread.agnostic_nms = True
        else:
            self.det_thread.agnostic_nms = False

    def chose_rtsp(self):
        self.rtsp_window = Window()
        config_file = 'config/ip.json'
        if not os.path.exists(config_file):
            ip = "rtsp://admin:123456@192.168.1.64:554"
            new_config = {"ip": ip}
    ...
    def chose_cam(self):
        try:
            ...
            MessageBox(
                self.closeButton, title='提示', text='请稍等，正在加载摄像头设备', time=2000, auto=True).exec_()
            # 自动检测本机有哪些摄像头
    ...
    def run_or_continue(self):
        ...
        if self.runButton.isChecked():
            ...
            self.statistic_msg('正在检测{}，当前模型：{}'.
                               format(source, os.path.basename(self.det_thread.weights)))
        else:
    ...
    def closeEvent(self, event):
        ...
        config['check_nms'] = self.checkBox_nms.checkState()
        config_json = json.dumps(config, ensure_ascii=False, indent=2)
        with open(config_file, 'w', encoding='utf-8') as f:
            f.write(config_json)
        MessageBox(
